...
class Rooms extends ControllerType
{
    ...
    function show()
    {
        ...
        $row_a = Adapter::fetch_object(Adapter::secure_query("SELECT * FROM rooms WHERE id = :roomid", [':roomid' => $roomId]));
	...
	$room  = new JsonRooms($row_a->id, EMULATOR_TYPE == 'plus' ? $row_a->caption : $row_a->name, $row_a->description, $row_a->users_max, $row_b->username, $row_b->id, $row_a->score, explode(',', $row_a->tags), $row_c == 1);

        header('Content-type: application/json');
	
        return str_replace("\\/", "/", json_encode($room));
    }
}
