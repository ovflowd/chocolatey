<table align="center" border="0" cellpadding="0" cellspacing="0" width="600" style="border-collapse:collapse">
    <tbody>
    <tr>
        <td class="m_-4152588659939001510container"
            style="color:#000000;font-family:'Ubuntu','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;border-collapse:collapse;padding:0 10px 0 10px">
            <table align="center" border="0" cellpadding="0" cellspacing="0" class="m_-4152588659939001510full"
                   style="border-collapse:collapse;width:100%">
                <tbody>
                <tr>
                    <td align="left" class="m_-4152588659939001510header"
                        style="color:#000000;font-family:'Ubuntu','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;border-collapse:collapse;padding:10px 0 0 0">
                        <img src="http://imgur.com/HAs98Uo.png" alt="Habbo" height="40" width="110" class="CToWUd">
                    </td>
                </tr>
                </tbody>
            </table>
            <table align="center" border="0" cellpadding="0" cellspacing="0" class="m_-4152588659939001510full"
                   style="border-collapse:collapse;width:100%">
                <tbody>
                <tr>
                    <td align="left" class="m_-4152588659939001510content"
                        style="color:#000000;font-family:'Ubuntu','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;border-collapse:collapse;padding:32px 0 24px 0">
                        <h1 id="m_-4152588659939001510hey-there-"
                            style="font-family:'Ubuntu Condensed','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;font-size:24px;font-weight:normal;line-height:1;margin:0;padding:0 0 24px 0">
                            Hey there!</h1>
                        <p style="color:#000000;font-family:'Ubuntu','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;font-size:16px;line-height:1.4;padding:0;margin:0 0 16px 0">
                            A yellow ducky told us that you have registered to Habbo with the email {{$mail}}
                        </p>
                        <p style="color:#000000;font-family:'Ubuntu','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;font-size:16px;line-height:1.4;margin:16px 0 16px 0;padding:0">
                            <a href="{{$url}}"
                               style="text-decoration:none;color:#ffffff;background-color:#00813e;border-radius:5px;display:inline-block;font-family:'Ubuntu Condensed','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;font-size:18px;padding:8px 24px"
                               target="_blank">Clickhere to securely activate your account</a></p>
                        <p style="color:#000000;font-family:'Ubuntu','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;font-size:16px;line-height:1.4;margin:16px 0 16px 0;padding:0">
                            – Habbo Staff</p>
                        <p style="color:#000000;font-family:'Ubuntu','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;font-size:16px;line-height:1.4;padding:0;margin:16px 0 0 0">
                            If you did not create this account yourself and do not want to receive emails from us
                            anymore, <a
                                    href="http://post.spmailtechnol.com/f/a/wLGQEv7u_hDPM6mwIdo4Og~~/AAD4FAA~/RgRaah0qP0EIASxEIMs2eXxXA3NwY1gEAAAAAFkNd2FybXVwX3Bvb2xfMWENd2FybXVwX3Bvb2xfMWAMNTIuMzcuMjAxLjk4SBdyZWdpc3RyYXRpb25fYWN0aXZhdGlvbkIKAASq6YhYqQod1VIPZ29vZ2xlQHVpb3Qub3JnCVEEAAAAAER-aHR0cHM6Ly93d3cuaGFiYm8uY29tL2lkZW50aXR5L3JlcG9ydF91bmF1dGhvcml6ZWRfdXNhZ2U_aGFzaD05Y2QzNWZhODg4NDUzOTBiNzkzNjFkZmMyNjM5YmJiMTMyY2IwNTA1JmVtYWlsPWdvb2dsZSU0MHVpb3Qub3JnRwJ7fRM~"
                                    style="color:#0f7dbc;text-decoration:none" target="_blank"
                                    data-saferedirecturl="https://www.google.com/url?hl=pt-BR&amp;q=http://post.spmailtechnol.com/f/a/wLGQEv7u_hDPM6mwIdo4Og~~/AAD4FAA~/RgRaah0qP0EIASxEIMs2eXxXA3NwY1gEAAAAAFkNd2FybXVwX3Bvb2xfMWENd2FybXVwX3Bvb2xfMWAMNTIuMzcuMjAxLjk4SBdyZWdpc3RyYXRpb25fYWN0aXZhdGlvbkIKAASq6YhYqQod1VIPZ29vZ2xlQHVpb3Qub3JnCVEEAAAAAER-aHR0cHM6Ly93d3cuaGFiYm8uY29tL2lkZW50aXR5L3JlcG9ydF91bmF1dGhvcml6ZWRfdXNhZ2U_aGFzaD05Y2QzNWZhODg4NDUzOTBiNzkzNjFkZmMyNjM5YmJiMTMyY2IwNTA1JmVtYWlsPWdvb2dsZSU0MHVpb3Qub3JnRwJ7fRM~&amp;source=gmail&amp;ust=1485455623071000&amp;usg=AFQjCNGCGHnxEDctcR7hUeEVWt1jyJMkAA">click
                                here</a>.</p>
                    </td>
                </tr>
                </tbody>
            </table>
            <table align="center" border="0" cellpadding="0" cellspacing="0" class="m_-4152588659939001510full"
                   style="border-collapse:collapse;width:100%">
                <tbody>
                <tr>
                    <td align="center" class="m_-4152588659939001510footer"
                        style="font-family:'Ubuntu','Trebuchet MS','Lucida Grande','Lucida Sans Unicode','Lucida Sans',Tahoma,sans-serif;border-collapse:collapse;color:#818a91;border-top:1px solid #aaaaaa;font-size:10px;line-height:1.4;padding:10px">
                        © 2017 Chocolatey
                    </td>
                </tr>
                </tbody>
            </table>
        </td>
    </tr>
    </tbody>
</table>